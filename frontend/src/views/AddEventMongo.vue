...
export default {
  ...
  methods: {
    ...
    getAllEmbassiesAndVenues() {
      return axios({
        method: "get",
        url: "//localhost:4040/mongo/allEmbassies",
        headers: {
          "auth-token": this.jwtToken,
        },
        data: {},
        ...
        .then((data) => {
          ...
          let venueFullList = {}
         data.data.forEach((el) => {
            embassyList.push({ name: el.name });
            venueFullList[el.name] = el.venues
          });
          this.embassyOptions = embassyList;
          this.venueFullList = venueFullList;
        })
        .catch((err) => {
          // Debugging
          console.log(err);
    ...
    if (this.newEventName === null || this.newEventName === "") {
      ...
      try {
        ...
        APIPayload.embassy = this.embassySelected.name
        if(this.venueAdded === 'Add') {
            APIPayload.venue = this.newVenueName
            APIPayload.seatingCapacity = this.newVenueCapacity
            APIPayload.addVenue = true

        }
        if(this.venueAdded === 'Use existing one') {
            APIPayload.venue = this.venueSelected.venueName
            APIPayload.seatingCapacity = null
        }
        APIPayload.event = this.newEventName
      ...
      return axios({
        method: "post",
        url: "//localhost:4040/mongo/addEvent",
        headers: {
          "auth-token": this.jwtToken,
        },
        data: APIPayload,
    ...
    venueOptions() {
      if (this.embassySelected === null) {
        ...
        this.venueFullList[this.embassySelected.name].forEach((el) => {
          venueOptions.push({
            name: el.name + " with seating capacity of " + el.seatingCapacity,
            venueName: el.name,
          });
        });
